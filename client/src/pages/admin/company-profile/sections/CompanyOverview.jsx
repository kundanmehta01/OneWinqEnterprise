...
export default function CompanyOverview({ company }) {
  ...
  const description = company?.description || 'OneWinq builds trusted digital identities for people and organizations through modern, connected experiences.';
  return <CompanySectionCard id="overview" title="Company Overview" icon={Building2}>
    ...
    <p className={`text-sm leading-6 text-slate-600 ${expanded ? '' : 'line-clamp-2'}`}>{description}</p>
