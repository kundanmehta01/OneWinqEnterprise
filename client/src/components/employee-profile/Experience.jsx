import React from 'react';
import { Briefcase, Plus, Trash2 } from 'lucide-react';
import { Button } from '../common/Button';
import { Input } from '../common/Input';

export const Experience = ({ experience = [], onChange }) => {
  const add = () => onChange([...experience, { title: '', company: '', description: '' }]);
  const remove = (idx) => onChange(experience.filter((_, i) => i !== idx));

  return (
    <div className="space-y-3 bg-white p-5 rounded-2xl border border-slate-100">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
          <Briefcase className="w-3.5 h-3.5" /> Experience
        </h3>
        <Button size="sm" variant="outline" icon={Plus} onClick={add}>Add</Button>
      </div>
      {experience.map((exp, idx) => (
        <div key={idx} className="p-3 border border-slate-100 rounded-xl space-y-2 bg-slate-50/50">
          <div className="grid grid-cols-2 gap-2">
            <Input
              label="Title"
              value={exp.title || ''}
              onChange={(e) => {
                const u = [...experience];
                u[idx] = { ...u[idx], title: e.target.value };
                onChange(u);
              }}
            />
            <Input
              label="Company"
              value={exp.company || ''}
              onChange={(e) => {
                const u = [...experience];
                u[idx] = { ...u[idx], company: e.target.value };
                onChange(u);
              }}
            />
          </div>
          <button type="button" onClick={() => remove(idx)} className="text-[10px] text-rose-500 hover:underline">
            Remove
          </button>
        </div>
      ))}
    </div>
  );
};
