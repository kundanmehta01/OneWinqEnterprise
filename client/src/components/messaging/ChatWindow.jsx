...
export const ChatWindow = ({ conversation, onBack, onOpenGroupInfo, onNewDirect, onNewGroup }) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-white">
      ...
      {/* Messages area */}
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 py-4 space-y-0.5 bg-gradient-to-b from-slate-50/30 to-white">
        {msgs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-400 py-16">
            <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center mb-3">
              <Send className="w-5 h-5 text-indigo-300" />
      ...
      {/* Input bar */}
      <div className="px-4 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] border-t border-slate-100 bg-white flex items-end gap-2 shrink-0">
        {/* File attach */}
        <button
          onClick={() => fileInputRef.current?.click()}
          className="p-2 rounded-xl text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-colors shrink-0 mb-0.5"
