...
export default function EditProfile() {
    ...
    employeeProfileService.getMyProfile()
      .then((profile) => {
        ...
        setFormData({
          ...
          department: profile.department?.name || profile.departmentId?.name || '',
          employeeId: profile.employeeId || profile._id || '',
          phone: profile.phone || profile.contactNumber || '',
          location: formatLocationStr(profile.location),
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await employeeProfileService.updateDraftProfile({
        bio: formData.bio,
  ...
  const handleSubmitForApproval = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Failed to submit for approval. Please try again.');
    } finally {
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isSubmittingApproval || isSubmitting}
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-sm transition-all disabled:opacity-60"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-sm transition-all disabled:opacity-60"
          >
      ...
      {savedSuccess && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center gap-3 text-emerald-800 text-xs font-semibold animate-in fade-in">
          ...
          <span>Profile saved successfully as draft.</span>
        </div>
      )}

      {submitSuccess && (
        <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-2xl flex items-center gap-3 text-indigo-800 text-xs font-semibold animate-in fade-in">
          <CheckCircle2 className="w-5 h-5 text-indigo-600 shrink-0" />
          <span>Profile submitted for admin approval! You'll be notified once it's reviewed.</span>
        </div>
      )}
