...
export const UserDashboard = () => {
  ...
  const [permissionAlert, setPermissionAlert] = useState(location.state?.message || '');

  useEffect(() => {
    if (location.state?.message) {
      setPermissionAlert(location.state.message);
    }
  }, [location.state]);
  ...
  // Format location safely (backend may return { city, country } object)
  const formatLocation = (loc) => {
    if (!loc) return 'Headquarters';
    if (typeof loc === 'string') return loc;
    if (typeof loc === 'object') {
      return [loc.city, loc.country].filter(Boolean).join(', ') || 'Headquarters';
    }
    return 'Headquarters';
  };
  ...
  const hero = {
    name: user?.name || member?.name || (user?.email ? user.email.split('@')[0] : 'Team Member'),
    designation: user?.designation || member?.designation || data?.hero?.designation || 'Team Member',
    role: user?.role || 'EMPLOYEE',
    department: user?.department || member?.departmentId?.name || data?.hero?.department || 'General',
    companyName: user?.company || member?.companyId?.name || 'OneWinq Enterprise',
    email: user?.email || '',
    location: formatLocation(member?.location || data?.hero?.location),
    slug: member?.profileId?.slug || (user?.email ? user.email.split('@')[0] : 'user')
  };

  const firstName = hero.name.split(' ')[0] || 'Alisha';
  const qrUrl = (!qrError && hero.slug) ? publicProfileService.getQrCodeUrl(hero.slug) : null;
  ...
  return (
    <div className="space-y-6">
      {/* Access Denied / Permission Flash Alert (if redirected from restricted area) */}
      {permissionAlert && (
        <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 flex items-center justify-between shadow-xs animate-in fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-amber-950">Access Denied</p>
              <p className="text-xs text-amber-800 mt-0.5">{permissionAlert}</p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => setPermissionAlert('')}
            className="p-1.5 rounded-lg hover:bg-amber-100 text-amber-700 transition cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-gradient-to-r from-[#0C1226] via-[#141C38] to-[#1C2750] rounded-3xl p-7 text-white relative overflow-hidden flex flex-col justify-between shadow-xl min-h-[220px]">
          ...
          <div className="relative z-10 max-w-xl">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-5">
              ...
              <span className="px-3 py-1.5 rounded-full bg-indigo-500/20 backdrop-blur-md border border-indigo-400/30 text-[11px] font-semibold text-indigo-200">
                {hero.role}
              </span>
              <span className="px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-[11px] font-semibold text-white">
                {hero.department}
        ...
        <div className="lg:col-span-4 bg-white rounded-3xl p-5 border border-slate-100 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/employee/card" className="text-[11px] font-bold text-indigo-600 hover:text-indigo-700">
              View All
            </Link>
          </div>
          ...
          <div className="rounded-2xl bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] p-4 text-white relative overflow-hidden shadow-md">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="px-2 py-0.5 rounded-full bg-indigo-500/30 border border-indigo-400/30 text-[9px] font-bold text-indigo-200 uppercase tracking-wider">
                {hero.role}
              </span>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[9px] text-indigo-300 font-mono truncate">{hero.email}</p>
              </div>
              {/* QR Code thumbnail */}
