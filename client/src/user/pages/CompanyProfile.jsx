...
export default function CompanyProfile() {
  ...
  const [company, setCompany] = useState({
    ...
    coreValues: [
      { title: 'Trust & Verification', desc: 'Every card and credential is cryptographically verified with enterprise integrity.' },
      { title: 'Innovation in Identity', desc: 'Merging NFC, dynamic QR codes, and AI-powered networking.' },
      { title: 'People-First Growth', desc: 'Empowering teams with seamless role and brand recognition across every interaction.' }
    ],
    locations: [
      { city: 'Indore, MP', address: 'Scheme No. 78, Indore, Madhya Pradesh 452010', type: 'Global Headquarters' }
    ]
  });

  useEffect(() => {
    userDirectoryService.getCompanyOverview()
      .then((data) => {
        if (data) {
          setCompany((prev) => ({
            ...prev,
            ...
            headquarters: typeof data.location === 'object'
              ? [data.location.address, data.location.city, data.location.state, data.location.country].filter(Boolean).join(', ')
              : (data.location || prev.headquarters),
            supportEmail: data.contact?.supportEmail || data.contact?.email || prev.supportEmail,
            founded: data.overviewStats?.foundedYear || prev.founded,
            employeeRange: data.overviewStats?.teamSize ? `${data.overviewStats.teamSize} Personnel` : prev.employeeRange,
            coreValues: Array.isArray(data.about?.values) && data.about.values.length > 0
              ? data.about.values.map(v => ({ title: v.title, desc: v.description }))
              : prev.coreValues
          }));
        }
      })
      .catch((err) => console.warn('Company overview fetch fallback:', err))
