...
const INITIAL_CONVERSATIONS = [
  {
    ...
    role: 'OneWinq Enterprise Administration',
    company: 'OneWinq Enterprise',
    location: 'Official System Broadcast',
    avatar: null,
    icon: Megaphone,
    status: 'Official Broadcast',
    category: 'announcement',
    lastMessage: 'All role-based modules & digital cards are live.',
    time: '9:00 AM',
    unread: 1,
    type: 'broadcast',
    starred: true,
    about: 'Official organization announcements and governance updates from Super Admin.',
    tags: ['Announcement', 'Company Wide', 'Governance'],
    messages: [
      { id: 1, sender: 'them', senderName: 'Super Admin', text: 'Welcome to OneWinq Enterprise platform. All employee identities, role routers, and profile approval workflows are active.', time: 'Yesterday' },
      { id: 2, sender: 'them', senderName: 'Super Admin', text: 'All-hands team alignment is scheduled for Friday. Please review your digital identity card profile.', time: '9:00 AM' }
    ]
  },
  {
    id: 101,
    name: 'Alex Morgan',
    role: 'VP of Product & Engineering (Manager)',
    company: 'OneWinq Enterprise',
    location: 'San Francisco, CA',
    avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=120&h=120&fit=crop&crop=face',
    status: 'Online',
    category: 'management',
    lastMessage: 'Let us review your deliverables at 2 PM.',
    time: '11:15 AM',
    unread: 0,
    type: 'direct',
    starred: true,
    about: 'Department Manager & Technical Leadership.',
    tags: ['Manager', '1-on-1', 'Deliverables'],
    messages: [
      { id: 1, sender: 'them', text: "Hello! Let's schedule our weekly 1-on-1 to align on your quarterly milestones and project roadmap.", time: '11:10 AM' },
      { id: 2, sender: 'me', text: "Sounds great! I have updated my profile and digital card deliverables.", time: '11:14 AM' },
      { id: 3, sender: 'them', text: 'Perfect, let us review your deliverables at 2 PM.', time: '11:15 AM' }
    ]
  },
  {
    id: 1,
    ...
    type: 'direct',
    starred: true,
    about: 'Architecture champion & core platform collaborator.',
    tags: ['Colleague', 'Engineering', 'API'],
    messages: [
      { id: 1, sender: 'them', text: 'Hey there! 👋', time: '11:20 AM' },
      { id: 2, sender: 'them', text: "Do you have the final version of the presentation for tomorrow's meeting?", time: '11:21 AM' },
      { id: 3, sender: 'me', text: "Hi Marcus! Yes, I've just updated it. Let me share the file with you.", time: '11:22 AM' },
      {
        id: 4,
        sender: 'me',
        isAttachment: true,
      ...
      { id: 5, sender: 'them', text: 'Looks great! 👍 Can we quickly discuss a few points before the meeting?', time: '11:23 AM' },
...
export default function Messages() {
  ...
  const [activeChatId, setActiveChatId] = useState(100);
  ...
  // Load real team members from backend to provide real employee-to-employee chats
  useEffect(() => {
    teamMemberService.getAll({ limit: 50 })
      .then((res) => {
        if (res?.members && Array.isArray(res.members)) {
          const myName = user?.name || '';
          const realTeammates = res.members
            .filter(m => m.name && m.name !== myName)
            .map((m, idx) => {
              const isLead = m.designation?.toLowerCase().includes('lead') ||
                m.designation?.toLowerCase().includes('manager') ||
                m.designation?.toLowerCase().includes('vp') ||
              ...
              return {
                ...
                department: m.departmentId?.name || 'Engineering',
                company: 'OneWinq Enterprise',
                location: 'Headquarters',
                avatar: m.avatarUrl || null,
                status: 'Active',
                category: isLead ? 'management' : 'colleague',
                lastMessage: `Connected on enterprise team directory.`,
                time: 'Recent',
                unread: 0,
                type: 'direct',
                starred: false,
                about: `${m.designation || 'Colleague'} in ${m.departmentId?.name || 'Engineering'} at OneWinq Enterprise.`,
                tags: [m.departmentId?.name || 'Department', isLead ? 'Management' : 'Colleague'],
                messages: [
                  {
                    id: 1,
                    sender: 'them',
                    text: `Hi ${user?.name ? user.name.split(' ')[0] : 'there'}! Happy to connect on OneWinq Enterprise.`,
                    time: '10:00 AM'
                  }
                ]
              };
          ...
          if (realTeammates.length > 0) {
            setConversations(prev => {
              ...
              const freshOnes = realTeammates.filter(rt => !existingNames.has(rt.name));
              return [...prev, ...freshOnes];
            });
          }
        }
      })
      .catch((err) => console.warn('Could not load dynamic colleagues for messages:', err));
  }, [user]);
  ...
  const filteredConversations = conversations.filter(c => {
    ...
    if (filterTab === 'Colleagues') return c.category === 'colleague' || c.type === 'direct';
    if (filterTab === 'Management') return c.category === 'management';
    if (filterTab === 'Announcements') return c.category === 'announcement' || c.type === 'broadcast';
    if (filterTab === 'Teams') return c.category === 'team' || c.type === 'group';
    return true;
  });
        ...
        <div className="lg:col-span-4 border-r border-gray-100 flex flex-col h-full bg-white">
          ...
          <div className="p-4 border-b border-gray-100">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
              {['All', 'Colleagues', 'Management', 'Announcements', 'Teams', 'Unread'].map(tab => (
                <button
                  key={tab}
